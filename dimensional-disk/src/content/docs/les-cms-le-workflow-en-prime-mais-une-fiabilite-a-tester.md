...
Les `CMS (Content Management System)`{.interpreted-text role="abbr"}, ainsi dénommés pour des raisons purement marketing, mais dont la fonction se comprend mieux avec l\'acronyme `GED (système de gestion
électronique de documents)`{.interpreted-text role="abbr"}, apportent des notions de *workflow* et de gestion des liens qui s\'avèrent précieuses lorsque l\'on gère des documents modulaires.
:::

S\'ils utilisent des formats monolithiques tels que **FrameMaker**, les rédacteurs techniques peuvent utiliser des `CMS (Content Management System)`{.interpreted-text role="abbr"} tels que SharePoint, Alfresco ou consorts pour :
...
[^1]: À chaque dépôt du fichier sur le `CMS (Content Management System)`{.interpreted-text role="abbr"}, et non à chaque enregistrement de son travail par le **rédacteur technique**.
