...
:   Par exemple, livre blanc du format structuré **DITA XML** au format cible PDF via `DITA-OT (DITA Open Toolkit)`.

Semi automatique

:   Par exemple, contenu au format **DITA XML** exporté en HTML puis collé sous un `CMS (Content Management System)`[^2].
...
[^2]: Ceci est automatisable par un script ; le `CMS (Content Management System)` Drupal propose également un module [DITA integration for Drupal]().