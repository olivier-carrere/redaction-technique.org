...
-   mettre en œuvre des *workflows* sous un `CMS (Content Management System)`{.interpreted-text role="abbr"},
...
Les fonctions principales d\'un `CMS (Content Management System)`{.interpreted-text role="abbr"} sont les suivantes :
...
Si le document est disponible en plusieurs langues, chaque ticket doit être dupliqué pour chaque langue ou, dans le cas d\'un `CMS (Content Management System)`{.interpreted-text role="abbr"}, à chaque langue doit correspondre un *workflow* distinct.
