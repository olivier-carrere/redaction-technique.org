...
4.  Pour générer les fichiers cibles, vous pouvez maintenant :
    ...
    c.  modifier le paramètre de langue `xml:lang`{.interpreted-text role="samp"} dans le fichier *ditamap* et dans les fichiers de contenu ,
...
-   Vos fichiers de contenu ont l\'extension `.dita`{.interpreted-text role="file"}.
...
-   Vos fichiers de contenu se trouvent dans des sous-répertoires des répertoires des versions linguistiques (par exemple, dans `fr_FR/tasks/`{.interpreted-text role="file"} et `fr_FR/topics/`{.interpreted-text role="file"}).
