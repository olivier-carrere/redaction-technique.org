...
quadrantChart
    accTitle: Formats sources selon la modularité et le format
    accDescr: Diagramme en quadrants positionnant FrameMaker, DocBook et DITA XML selon leur degré de modularité (monolithique à modulaire) et leur format de fichier (binaire à texte). FrameMaker est binaire et monolithique ; DocBook est un format texte moyennement modulaire ; DITA XML est un format texte fortement modulaire.
    title Formats sources : degré de modularité et format
    x-axis Monolithique --> Modulaire
    y-axis Binaire --> Texte
    quadrant-1 Formats texte modulaires
    quadrant-2 Formats texte monolithiques
    quadrant-3 Formats binaires monolithiques
    quadrant-4 Formats binaires modulaires
    FrameMaker: [0.26, 0.27]
    DocBook: [0.26, 0.76]
    DITA XML: [0.80, 0.79]`} />
...
Dans une logique livre vers aide en ligne, on rédige d'abord un document linéaire complet dont on dérive ensuite les rubriques d'aide individuelles ; dans une logique aide en ligne vers livre, on rédige d'abord des rubriques autonomes et atomiques que l'on assemble ensuite en un document de type livre lorsque c'est nécessaire. Les formats monolithiques se prêtent naturellement à la première, les formats modulaires à la seconde.

Indépendamment du contrôle de la mise en page, les formats diffèrent aussi selon qu'ils sont **structurés sémantiquement** — selon que le balisage encode le sens plutôt que l'apparence :

| Format     | Structuré (sémantique)               |
| ---------- | ------------------------------------ |
| FrameMaker       | Non                                  |
| DocBook          | Oui                                  |
| DITA XML         | Oui                                  |
...
graph TD
    accTitle: Format source de rédaction technique monolithique
    accDescr: Schéma montrant deux documents monolithiques autonomes, sans contenu partagé entre eux.

    subgraph mono["Fichiers monolithiques"]
        direction LR
        A["Document A"]
        B["Document B"]
    end
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif", "clusterBkg": "{{childFill}}", "clusterBorder": "{{childStroke}}"}}}%%
graph TD
    accTitle: Format source de rédaction technique modulaire
    accDescr: Schéma montrant des grappes de fichiers modulaires réutilisées entre deux documents, Document A et Document B, qui partagent du contenu au lieu de le dupliquer.

    subgraph cluster["Grappes de fichiers modulaires"]
        direction LR
        M1[Module]
        M2[Module]
        M3[Module]
