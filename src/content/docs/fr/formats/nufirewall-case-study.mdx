...
graph TB
    accTitle: Les phrases se découpent différemment selon les langues
    accDescr: Schéma montrant les mêmes segments conref assemblés dans un ordre différent pour deux langues, illustrant pourquoi le conref au niveau du segment ne se traduit pas proprement.

    subgraph langA["Langue A"]
        direction LR
        A1["conref A"] --> A2["conref B"] --> A3["conref C"]
    end

    subgraph langB["Langue B"]
        direction LR
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph TD
    accTitle: Imbriquer les conref sur plusieurs niveaux est risqué
    accDescr: Schéma montrant trois niveaux d'imbrication de conref, un conref pointant vers un conref pointant vers un autre conref.
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif", "clusterBkg": "{{childFill}}", "clusterBorder": "{{childStroke}}"}}}%%
graph TD
    accTitle: Les conref modularisent de petits blocs d'information
    accDescr: Schéma opposant une source monolithique sans réutilisation à une source modulaire composée de plusieurs blocs conref.

    subgraph mono["Source monolithique"]
        M[Contenu]
    end

    subgraph mod["Source modulaire"]
        direction TB
        C1[conref]
        C2[conref]
        C3[conref]
