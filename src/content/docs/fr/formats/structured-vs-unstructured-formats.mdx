...
graph TD
    accTitle: Organisation du contenu structurée ou non structurée
    accDescr: Schéma comparant des sections non structurées, qui restent des blocs indifférenciés, à des sections structurées, chacune décomposée en types d'information concept, task et reference.

    subgraph un["Non structuré"]
        direction TB
        U1["Section 1"]
        U2["Section 2"]
    end

    subgraph str["Structuré"]
        direction TB
...
graph TD
    accTitle: RAW vers TIFF vers JPEG, un processus non réversible
    accDescr: Schéma montrant une chaîne à sens unique de RAW vers TIFF vers JPEG, chaque étape perdant des informations qui ne peuvent pas être récupérées en inversant le processus.
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph TD
    accTitle: DITA XML vers DocBook vers PDF, un processus non réversible
    accDescr: Schéma montrant une chaîne à sens unique de DITA XML vers DocBook vers PDF, chaque étape sémantiquement plus pauvre que la précédente et non récupérable en inversant le processus.
...
{/* TODO : « FrameMaker Structured 11 » (2012) et les chemins de menus ci-dessous sont obsolètes ; à actualiser pour une version récente de FrameMaker lors de la prochaine révision de cette section. */}
...
gantt
    accTitle: Migration de FrameMaker vers DITA XML
    accDescr: Schéma montrant la migration vers DITA XML et la mise en place de la chaîne DITA XML comme deux activités menées en parallèle.
    dateFormat YYYY-MM-DD
    axisFormat %d
    section Migration
    Migration vers DITA XML                    :m1, 2024-01-01, 3d
    Mise en place de la chaîne DITA XML        :m2, 2024-01-01, 3d`} />
**Migration de FrameMaker vers DITA XML**
...
<Mermaid code={`%%{init: {"themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif", "taskBkgColor": "{{childFill}}", "taskBorderColor": "{{childStroke}}", "taskTextColor": "{{childText}}", "taskTextOutsideColor": "{{childText}}", "taskTextLightColor": "{{childText}}", "taskTextDarkColor": "{{childText}}", "sectionBkgColor": "{{parentFill}}", "sectionBkgColor2": "{{childFill}}", "gridColor": "{{lineColor}}"}}}%%
gantt
    accTitle: Restructuration du contenu FrameMaker et mise en place de la chaîne DITA XML
    accDescr: Schéma montrant la restructuration du contenu FrameMaker menée en parallèle de la mise en place de la chaîne DITA XML.
    dateFormat YYYY-MM-DD
    axisFormat %d
    section Restructuration
    Restructuration du contenu FrameMaker      :r1, 2024-01-01, 3d
    Mise en place de la chaîne DITA XML        :r2, 2024-01-01, 3d`} />
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph TD
    accTitle: Application d'une table de conversion de FrameMaker vers DITA XML
    accDescr: Schéma montrant le contenu FrameMaker converti en DITA XML par l'application d'une table de conversion.

    A[FrameMaker] -->|table de conversion| B["DITA XML"]
...
Le cœur de traitement de texte s'exécute en direct ci-dessous. L'entrée tient lieu de la sortie de `ack "<title>" *.dita`, et `tr` est réécrit avec le `\L` de sed. Modifiez les lignes et observez les commandes `mv` changer :

<DitaRenameBox lang="fr" />
