...
![Tri d'une vaste collection de photos pour l'archive centralisée d'une association](/assets/pile-photos.jpg)
...
graph LR
    accTitle: Schéma du flux de travail de la galerie photo
    accDescr: Schéma montrant les photographes téléversant et les iconographes sélectionnant, classant et indexant les photos dans Piwigo, qui alimente un dépôt Git sauvegardé localement et cloné par les membres de l'association, et permettant aux graphistes de rechercher et télécharger des photos.

    Photographe -->|Upload| Piwigo
    Iconographe -->|Sélection, classement, indexation| Piwigo
    Piwigo --> Git["Dépôt Git"]
    Git --> Sauvegarde["Sauvegarde locale"]
    Git -->|Clone| Membre["Membre de l'association"]
    Piwigo -->|Recherche, téléchargement| Graphiste
    ...
    class Piwigo hub
    class Photographe,Iconographe,Git,Sauvegarde,Membre,Graphiste node
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph TD
    accTitle: Commande find Bash listant les fichiers photo dans les sous-répertoires filtrés par taille
    accDescr: Décomposition de la commande "find . -size +1M -exec mv ./repertoire-plus-de-1mo/ {} \\;" token par token, avec une explication de chaque partie.

    A["$ : Invite de la ligne de commande"] --> B["find : Recherche des fichiers dans une hiérarchie de répertoires"]
    B --> C[". : Répertoire en cours"]
    C --> D["-size +1M : Ne prend en compte que les fichiers d'au moins 1 Mo"]
    D --> E["-exec : Exécute la commande qui suit"]
    E --> F["mv : Déplace les fichiers"]
    F --> G["./repertoire-plus-de-1mo/ : Répertoire cible"]
    G --> H["{} : Remplacé par les noms de fichiers"]
    H --> I["\\; : Met fin à l'exécution de la commande"]
...
![Galerie photo en ligne Piwigo centralisée dans le cloud pour les membres de l'association (photo Kevin Harber)](/assets/galerie-photo.jpg)
...
## Définition des rôles des membres du workflow photo
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph LR
    accTitle: Flux de gestion des photos Piwigo par rôle
    accDescr: Schéma montrant le photographe prenant la photo, la retouchant et la téléversant, l'iconographe sélectionnant, classant et ajoutant des mots-clés, et le graphiste consultant, téléchargeant et publiant les photos.

    Photographe --> Prise["Prise de vue"] --> Retouche --> Upload --> Selection["Sélection et classement"]
    Iconographe --> Selection
    Selection --> Motscles["Ajout de mots-clés"]
    Motscles --> Consultation["Consultation et téléchargement"]
    Graphiste --> Consultation
    Consultation --> Publication

    classDef role fill:{{parentFill}},stroke:{{parentStroke}},stroke-width:2px,color:{{parentText}},font-weight:600
    classDef step fill:{{childFill}},stroke:{{childStroke}},stroke-width:1.5px,color:{{childText}},font-weight:500

    class Photographe,Iconographe,Graphiste role
    class Prise,Retouche,Upload,Selection,Motscles,Consultation,Publication step
...
![Classe leaflet LaTeX montrant six configurations possibles de dépliant à trois volets](/assets/leaflet-pliage.png)
...
![Rendu PDF généré à partir du code source LaTeX avec césure automatique et typographie précise](/assets/latex-rendu.png)
...
![Éditeur LaTeX WYSIWYG Gummi avec aperçu PDF en direct à côté du code source LaTeX](/assets/latex-wysiwyg-gummi.png)
:::
...
Le suivi des modifications, que ce soit sous Overleaf ou Git, prévient efficacement les erreurs : il est très facile de visualiser les modifications de fond ou de forme entre deux versions, de revenir à tout moment à une version précédente, de maintenir en parallèle plusieurs versions, etc.
...
![Comparaison visuelle de deux versions PDF compilées d'un document LaTeX montrant les modifications de mise en page et de contenu](/assets/latex-diff-pdf.png)
...
Et car il s'agit de s'amuser en procédant à ces tâches, j'ai joué sur un côté décalé, quitte à revenir parfois sur certains choix, dont l'humour était trop obscur... La photo suivante, par exemple, semblait ne faire rire que moi et ne pas apporter grand-chose à la compréhension de la section *Didacticiels* :

![Frances Densmore enregistrant Mountain Chief, photo historique libre de droits utilisée comme iconographie dans la documentation](/assets/Frances_Densmore_recording_Mountain_Chief2.jpg)
