...
![Commande find Bash listant les fichiers photo dans les sous-répertoires filtrés par taille](/assets/find-command.svg)
...
<Mermaid code={`%%{init: {"themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif", "archEdgeColor": "{{lineColor}}", "archEdgeArrowColor": "{{lineColor}}", "archGroupBorderColor": "{{childStroke}}"}}}%%
architecture-beta
    service piwigo(server)[Serveur Piwigo]
    service db(database)[Base de données]
    service disque(disk)[Copie locale Git LFS]
    service gitlab(cloud)[GitLab]
    service membres(internet)[Membres de l'association]

    piwigo:B -- T:db
    piwigo:R -- L:disque
    disque:R --> L:gitlab
    gitlab:R --> L:membres`} />
**Hébergement centralisé, sauvegarde décentralisée.**
...
<Mermaid code={`%%{init: {"themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif", "clusterBkg": "{{childFill}}", "clusterBorder": "{{childStroke}}"}}}%%
swimlane-beta
    accTitle: Flux de gestion des photos Piwigo par rôle
    accDescr: Trois couloirs représentant le photographe, l'iconographe et le graphiste. Le photographe prend la photo, la retouche et la téléverse ; l'iconographe sélectionne, classe et ajoute des mots-clés ; le graphiste consulte, télécharge et publie les photos.

    subgraph photographe["Photographe"]
        prise["Prise de vue"]
        retouche["Retouche"]
        upload["Upload"]
        prise --> retouche --> upload
    end

    subgraph iconographe["Iconographe"]
        selection["Sélection et classement"]
        motscles["Ajout de mots-clés"]
        selection --> motscles
    end

    subgraph graphiste["Graphiste"]
        consultation["Consultation et téléchargement"]
        publication["Publication"]
        consultation --> publication
    end

    upload --> selection
    motscles --> consultation

    classDef step fill:{{parentFill}},stroke:{{parentStroke}},stroke-width:1.5px,color:{{parentText}},font-weight:500
