...
La génération à partir de données SQL conserve les données produit dans une base et génère le texte correspondant au moment de la génération. La chaîne utilisée ici (Python, SQLite, Jinja et reStructuredText) n'est qu'une mise en œuvre de ce principe ; voir [base de données SQL](/fr/tech-writing-process/sql-database/) pour comprendre pourquoi le texte lui-même a intérêt à rester dans des fichiers.
:::
