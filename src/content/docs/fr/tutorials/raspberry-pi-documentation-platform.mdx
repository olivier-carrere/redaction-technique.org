...
:::note[Étude de cas]
Cette page est une étude de cas : une démonstration de faisabilité montrant qu'une chaîne documentaire complète (Sphinx, reStructuredText et Git) peut fonctionner sur un matériel très modeste. Ce n'est pas une recommandation de poste de travail ou de plateforme de publication actuelle. Pour les pratiques actuelles, voir [adopter le docs-as-code](/fr/docs-as-code/).
:::
...
:::note
Ce tutoriel a été rédigé et testé sur un Raspberry Pi 3. Il n'a pas été vérifié sur les modèles plus récents ; les noms de paquets, les menus et les commandes peuvent différer selon la carte ou la version du système d'exploitation.
:::

:::note
Les buts de ce billet sont de :

- Présenter un <abbr title="Proof of Concept, démonstration de faisabilité">POC</abbr> et utiliser des ressources minimales pour créer, gérer et publier une documentation professionnelle. La plupart des opérations se déroulent donc en mode texte, sous Linux, mais les mêmes outils fonctionnent également en mode graphique.
...
- Carte micro-SD de 32 Go classe 10 (de préférence).
- Connexion Internet filaire ou Wi-Fi.

1. Installez la distribution Linux Raspberry Pi OS sur votre Raspberry Pi 3 à l'aide de Raspberry Pi Imager.
...
1. Modifiez un fichier source modulaire de ce blog :
   ...
   - ou à l'aide d'un éditeur en ligne de commande, par exemple :
...
1. Modifiez un fichier source des images de ce blog :
   ...
   - ou à l'aide d'un éditeur en ligne de commande :
...
2. Affichez l'historique des modifications des sources de ce blog :
   ...
   ![Un commit atomique s'étendant sur une bonne quinzaine de fichiers](/assets/historique-git-redaction-technique.png)
...
2. Générez la dernière version taguée de ce blog aux formats PDF, HTML et EPUB :
...
Les prochains tests consisteront à générer la version DITA XML de ce blog, et à générer ce blog sur un smartphone en installant une distribution Linux sur Android.
