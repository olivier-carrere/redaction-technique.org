...
description: "Tutoriels DITA XML et XSL-FO : créez une documentation topic par topic et appliquez des mises en page professionnelles pour la sortie PDF et l'impression."
goal: [structured-authoring]
contentType: task
---

:::note[Chaîne d'outils historique]
Ces didacticiels documentent une architecture de publication précise : des sources DITA XML transformées en PDF et en d'autres formats au moyen de feuilles de style XSL-FO. Ils restent utiles si vous maintenez ou personnalisez une chaîne de ce type. Pour choisir un modèle de rédaction pour un nouveau projet, voir [choisir un modèle de rédaction](/fr/tools-and-formats/).
:::
...
Par défaut, <abbr title="DITA Open Toolkit">DITA-OT</abbr>  n'insère pas automatiquement dans les fichiers PDF le texte *Exemple :* devant le titre d'un exemple contenu entre balises DITA XML `<example>`. La syntaxe XSL-FO offre cependant cette possibilité.
...
Par défaut, cependant, ce contenu sera structuré comme suit dans le PDF par <abbr title="DITA Open Toolkit">DITA-OT</abbr>  :
...
1.  Remplacez dans la feuille de style `plugins/org.dita.pdf2/xsl/fo/commons.xsl` (sous <abbr title="DITA Open Toolkit">DITA-OT</abbr>  1.7.) le template suivant :
...
- Java 17 ou version ultérieure (JDK),
- connexion Internet.
...
Ce didacticiel DITA XML est destiné à vous guider dans la mise en place et l'utilisation de la chaîne de publication <abbr title="DITA Open Toolkit">DITA-OT</abbr> sur une installation Windows actuelle.
...
2.  Téléchargez DITA Open Toolkit 4.4 (`dita-ot-4.4.zip`), puis décompressez l'archive, par exemple sur le bureau.

3.  Ouvrez le menu `Démarrer`, saisissez `cmd`, puis appuyez sur `Entrée`. Un terminal apparaît.

4.  Dans le terminal, placez-vous dans le répertoire décompressé :

    ```console
    cd Bureau\dita-ot-4.4
    ```

5.  Générez votre premier PDF :
    ...
    Félicitations, vous avez compilé votre premier projet DITA XML ! Vous trouverez le fichier cible dans le répertoire `out`. Vous pouvez maintenant compiler d'autres projets en ignorant les étapes 1 et 2.
...
## Afficher un index dans un PDF

Vous avez méticuleusement inséré vos entrées d'index dans vos fichiers de contenu DITA XML et vous souhaitez qu'elles apparaissent dans le PDF généré.

Historiquement, le moteur Apache FOP fourni avec <abbr title="DITA Open Toolkit">DITA-OT</abbr> ne prenait pas en charge la génération des index : afficher un index imposait alors un moteur FO commercial (tel que XEP de RenderX, utilisé par des outils propriétaires comme XMetaL) ou l'un de plusieurs contournements. Ce n'est plus le cas : la génération des index a été refondue dans DITA-OT 2.2 et l'Apache FOP fourni produit désormais les index directement. Avec une version actuelle de <abbr title="DITA Open Toolkit">DITA-OT</abbr>, les entrées d'index que vous insérez dans votre contenu DITA XML sont incluses dans le PDF, sans moteur supplémentaire ni contournement.

Si vous choisissez malgré tout de ne pas afficher d'index dans le PDF, cela reste une option légitime : les index sont difficiles à maintenir et offrent un surplus d'utilisabilité discutable dans un document qui ne sera consulté que marginalement sous forme imprimée. Et pour un format électronique, l'absence d'un index est largement compensée par la fonction de recherche en plein texte.

## Utiliser l’IDE nXML pour DITA XML

<a id="utiliser-ide-nxml-pour-dita-xml"></a>

Le mode nXML propose de valider en temps réel les documents XML (DocBook, XHTML et autres) et d’autocompléter les balises XML selon le contexte, sans qu’il soit besoin de connaître le schéma par cœur. Il est intégré au cœur d’Emacs depuis Emacs 23 : il n’y a donc rien à installer, tout Emacs actuel le contient déjà. Par défaut, cependant, le mode nXML ne connaît pas DITA XML ; ce didacticiel vous permet de l’activer pour DITA XML en associant les schémas RelaxNG de DITA.

**Prérequis**

- Emacs 23 ou version ultérieure (toute version actuelle ; le mode nXML est inclus)
...
2. Le mode nXML fait déjà partie d’Emacs : il n’y a donc rien à télécharger ni à corriger. C’est le mode par défaut des fichiers `.xml` ; pour l’utiliser également sur les fichiers de contenu DITA, indiquez à Emacs d’ouvrir les fichiers `.dita` en mode nXML en ajoutant la ligne suivante à votre fichier `.emacs` (ou `init.el`) :
...
5. Téléchargez l’archive des fichiers schemas.xml dans le répertoire racine de votre projet de documentation DITA XML, puis collez la suite de commandes ci-dessous en remplaçant `<répertoire de langue>` par la valeur appropriée, en_US, ou fr_FR, par exemple. Répétez cette étape pour tous vos répertoires de langue.
    ...
    Ces fichiers `schemas.xml` sont les fichiers de localisation de schémas du mode nXML : ils indiquent à nXML quel schéma RelaxNG appliquer à chaque type de *topic* DITA, ce qui active la validation et l’autocomplétion. Pour associer manuellement un schéma au tampon courant, utilisez plutôt `C-c C-s C-f` (`rng-set-schema-file-name`).
...
:::note[Note]
Le paquet Predictive ne semble plus maintenu : sa dernière version (0.24) date de février 2013. Les instructions ci-dessous sont conservées à titre de référence — vérifiez l’état actuel de l’outil avant d’y investir du temps. Pour une alternative légère et directement utilisable, l’auteur du paquet suggère le mode `pabbrev`.
:::
...
- [Cas concret : documentation de NuFirewall](../../formats/nufirewall-case-study/)
