...
Ce didacticiel génère des topics DITA XML à partir de données, au moyen d'un script Python. Il est utile si vous maintenez un contenu DITA dont les données produit (noms, versions) changent souvent ; la même approche pour reStructuredText est présentée dans [insérer automatiquement des données dans un fichier reStructuredText](/fr/tutorials/auto-insert-data-restructuredtext/).
:::
