...
description: "Validate technical documentation before delivery: run validation in parallel with writing, track every change request through a CMS workflow or a ticketing system, and use comparison tools to review updates reliably."
...
Content must be validated before [delivery](../delivery/). This may seem obvious, but it requires the involvement of the people in charge of validation from the outset. Ideally, the validation phase takes place in parallel with the creation phase: early modifications are less costly.
...
When documentation sources are text files in Git, the pull request provides the traceability described above (what changed, who changed it, when, and who approved it) and version comparison becomes a plain `git diff`, rather than an export to a word processor. Validation then splits into two complementary layers:

- **Automated checks** run on every change before it is merged: the build itself, link checking, metadata and structure validation. They catch mechanical errors consistently and cheaply. See the [CI/CD pipeline example](../../toolkit/example-cicd-pipeline/).
- **Human review** covers what no tool can check: technical accuracy against the product, and editorial quality. See the [review workflow example](../../toolkit/example-review-workflow/), the [technical review checklist](../../toolkit/technical-review-checklist/), and the [documentation quality checklist](../../toolkit/documentation-quality-checklist/).

Content drafted with AI assistance goes through the same two layers; it does not need a separate process, but it does need the human review.
