...
Source management systems offer the option of creating branches for a project. If a project splits into two incompatible projects, a branch is created from the main project. This allows the technical writer to manage various [translations](../translation/) of technical documentation.
...
The main difference between [Git](../git-from-file-to-content/) and Subversion is the way they handle branches. Creating a branch in Subversion is equivalent to duplicating a directory. The files in these directories then evolve independently. In Git, however, branches can be created without duplicating data. Within the same local directory, you can use a command to switch between branches.
...
- [Example: a basic docs-as-code workflow](../../toolkit/example-docs-as-code-workflow/)
