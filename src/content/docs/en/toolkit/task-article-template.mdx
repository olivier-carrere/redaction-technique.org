...
:::tip[Template]
This is a **starting point to copy and adapt**, not a worked example. For a filled-in version, see the [example docs-as-code workflow](../example-docs-as-code-workflow/). To learn how tasks fit into information architecture, read [Information types](../information-types/).
:::

Use this structure when the reader needs to **do** something with a concrete, checkable outcome — not for background explanation (use the [concept template](../concept-article-template/)) or lookup specifications (use the [reference template](../reference-article-template/)).
