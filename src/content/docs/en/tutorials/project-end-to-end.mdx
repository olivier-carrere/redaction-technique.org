...
![Sorting a large photo collection for a non-profit's centralized archive](/assets/pile-photos.jpg)
...
graph TD
    accTitle: Photo gallery workflow diagram
    accDescr: Diagram showing photographers uploading and iconographers selecting, classifying, and indexing photos into Piwigo, which feeds a Git repository backed up locally and cloned by non-profit members, and lets graphic designers search and download photos.

    Photographer -->|Upload| Piwigo
    Iconographer -->|Selection, classification, indexing| Piwigo
    Piwigo --> Git[Git repository]
    Git --> Backup[Local backup]
    Git -->|Clone| Member["Non-profit member"]
    Piwigo -->|Search, download| Designer["Graphic designer"]
    ...
    class Piwigo hub
    class Photographer,Iconographer,Git,Backup,Member,Designer node
...
![Bash find command listing photo files across subdirectories filtered by size](/assets/en/find-command.svg)
...
![Piwigo online photo gallery centralized in the cloud for non-profit members (photo Kevin Harber)](/assets/galerie-photo.jpg)
...
| Storage      | Unlimited                      | 250 GB                                 | 100 GB           |
...
architecture-beta
    service piwigo(server)[Piwigo server]
    service db(database)[Database]
    service disk(disk)[Local Git LFS copy]
    service gitlab(cloud)[GitLab]
    service members(internet)[Non-profit members]

    piwigo:B -- T:db
    piwigo:R -- L:disk
    disk:R --> L:gitlab
    gitlab:R --> L:members`} />
**Centralized hosting, decentralized backup.**
...
swimlane-beta
    accTitle: Piwigo photo management workflow by role
    accDescr: Three lanes for the photographer, the iconographer, and the graphic designer. The photographer shoots, edits, and uploads the photo; the iconographer selects, classifies, and adds keywords; the graphic designer views, downloads, and publishes the photos.

    subgraph photographer["Photographer"]
        shooting["Shooting"]
        editing["Editing"]
        upload["Upload"]
        shooting --> editing --> upload
    end

    subgraph iconographer["Iconographer"]
        selection["Selection and classification"]
        keywords["Adding keywords"]
        selection --> keywords
    end

    subgraph designer["Graphic designer"]
        viewing["Viewing and downloading"]
    ...
    class shooting,editing,upload,selection,keywords,viewing,publishing step
...
![LaTeX leaflet class showing six possible tri-fold brochure configurations](/assets/leaflet-pliage.png)
...
\begin{enumerate}[itemsep=0mm,leftmargin=*]
   ...
   \item Add keywords to photos:

     \begin{itemize}
       \item Indicate \emph{Print} if the photo is suitable for printing, \emph{Web} if not.
       \item To delete the photo, enter \emph{Delete}. It will be deleted later.\footnote{It will be kept in the backup.}
     \end{itemize}
...
![PDF output rendered from LaTeX source code with automatic hyphenation and precise typography](/assets/latex-rendu.png)
...
![Gummi WYSIWYG LaTeX editor with live PDF preview alongside LaTeX source code](/assets/latex-wysiwyg-gummi.png)
:::
...
![GitHub diff view showing LaTeX background color changes between document versions](/assets/latex-historique-fond-github.png)
**Viewing background changes in GitHub**

![GitHub diff view showing LaTeX typographic style modifications between document versions](/assets/latex-history-form-github.png)
**Viewing form modifications in GitHub**

Here’s the evolution of an excerpt from the compiled PDF:

![Visual diff comparing two compiled PDF versions of a LaTeX document showing layout and content changes](/assets/latex-diff-pdf.png)
...
![Frances Densmore recording Mountain Chief, a royalty-free historical photo used as iconography in documentation](/assets/Frances_Densmore_recording_Mountain_Chief2.jpg)
...
- [Project definition](../../tech-writing-process/project-definition/)

## Related reading

- [Turn Word files into SEO-optimized web pages: boost your content reach with AI](https://redaction-technique.org/blog/turn-word-files-seo-optimized-web-pages-ai) — an end-to-end pipeline from source documents to published web pages.
