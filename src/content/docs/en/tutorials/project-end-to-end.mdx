...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph TD
    accTitle: Bash find command listing photo files across subdirectories filtered by size
    accDescr: Breakdown of the command "find . -size +1M -exec mv ./directory-over-1mb/ {} \\;" token by token, explaining each part.

    A["$: Command prompt"] --> B["find: Searches for files in a directory hierarchy"]
    B --> C[".: Current dir"]
    C --> D["-size +1M: Only considers files that are at least 1 MB"]
    D --> E["-exec: Runs the following command"]
    E --> F["mv: Moves files"]
    F --> G["./directory-over-1mb/: Target dir"]
    G --> H["{}: Replaced with filenames"]
    H --> I["\\;: Terminates command"]
...
graph LR
    ...
    accDescr: Diagram showing the photographer shooting, editing, and uploading photos, the iconographer selecting, classifying, and adding keywords, and the graphic designer viewing, downloading, and publishing them.

    Photographer --> Shooting --> Editing --> Upload --> Selection["Selection and classification"]
    Iconographer --> Selection
    Selection --> Keywords["Adding keywords"]
    Keywords --> Viewing["Viewing and downloading"]
    Designer["Graphic designer"] --> Viewing
    Viewing --> Publishing
    ...
    class Photographer,Iconographer,Designer role
