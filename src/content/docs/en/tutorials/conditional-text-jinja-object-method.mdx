...
description: "Profile content with Jinja by defining objects (audience, platform, version) to include or exclude blocks according to their attributes using a Python profiling script."
...
With Jinja, you can define objects (audience, platform, version, etc.) and include or exclude text blocks according to their attributes. For a simpler introductory approach, see [creating different documents from the same sources using Jinja](../conditional-text-jinja/). The same conditional-text concept also applies to [DITA XML's ditaval mechanism](../../formats/nufirewall-case-study/).
...
6. Create a condition for audiences other than electricians or plumbers:
    ...
    .. admonition:: No danger
       ...
       If you're not a plumber or an electrician, you're not in any danger.
...
10. Use a more readable variant in the content file:
    ...
    It's more economical to use a single object class, even if it does mix apples and oranges (as much in this far-fetched example as in real life, where you'd mix audiences, versions, platforms, etc.).
...
11. Modify your script to indicate the public and the season as parameters:
    ...
    if len(sys.argv) == 3:
    	...
    	print('''Accepted values:\nelectrician or plumber as the 1st parameter\nwinter,
            spring, summer, autumn as the 2nd parameter''')
    else:
        print('Please indicate the audience and the season')
    ```
...
- [Create different documents from the same ReST sources (conditional text)](../conditional-text-sphinx-rest/)
