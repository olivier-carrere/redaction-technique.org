...
description: "When a company industrializes technical writing, the key issue is not the tool but the underlying format."
---
...
If we think in terms of formats, on the other hand, it's possible to implement scalable solutions. An open format such as OpenDocument or **DITA XML**, for example (only the latter being an industrial format for technical writing), is not tied to a given tool. It can therefore be modified and manipulated using a variety of software programs. Structured formats such as **DocBook** and **DITA XML**, linked to a standardized XSD schema, can be easily managed, from creation to publication, using a whole range of tools, from free text editors to proprietary graphic software suites.
