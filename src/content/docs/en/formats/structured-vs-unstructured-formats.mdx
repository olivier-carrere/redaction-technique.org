...
goal: [structured-authoring, learn]
contentType: concept
---

import DitaRenameBox from '../../../../components/DitaRenameBox.astro';
...
graph TD
    accTitle: Structured versus unstructured content organization
    accDescr: Diagram comparing unstructured sections, which remain undifferentiated blocks, with structured sections, each broken down into concept, task, and reference information types.

    subgraph un["Unstructured"]
        direction TB
        U1[Section 1]
        U2[Section 2]
    end
    ...
    subgraph str["Structured"]
        direction TB
        S1[Section 1] --> C1[concept]
        S1 --> T1[task]
        S1 --> R1[reference]
        S2[Section 2] --> C2[concept]
        S2 --> T2[task]
        S2 --> R2[reference]
    end

    classDef section fill:{{parentFill}},stroke:{{parentStroke}},stroke-width:2px,color:{{parentText}},font-weight:600
    classDef itype fill:{{childFill}},stroke:{{childStroke}},stroke-width:1.5px,color:{{childText}},font-weight:500

    class U1,U2,S1,S2 section
    class C1,T1,R1,C2,T2,R2 itype
...
graph TD
    accTitle: RAW to TIFF to JPEG is a non-reversible process
    accDescr: Diagram showing a one-way chain from RAW to TIFF to JPEG, each step losing information that cannot be recovered by reversing the process.
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph TD
    accTitle: DITA XML to DocBook to PDF is a non-reversible process
    accDescr: Diagram showing a one-way chain from DITA XML to DocBook to PDF, each step semantically poorer than the last and not recoverable by reversing the process.

    A[DITA XML] --> B[DocBook] --> C[PDF]
...
{/* TODO: "FrameMaker Structured 11" (2012) and the menu paths below are outdated; refresh for a current FrameMaker release when this section is next revised. */}
...
gantt
    accTitle: Migration from FrameMaker to DITA XML
    accDescr: Diagram showing the migration to DITA XML and the implementation of the DITA XML workflow running as two parallel activities.
    dateFormat YYYY-MM-DD
    axisFormat %d
    section Migration
    Migration to DITA XML                    :m1, 2024-01-01, 3d
    Implementation of the DITA XML workflow  :m2, 2024-01-01, 3d`} />
...
<Mermaid code={`%%{init: {"themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif", "taskBkgColor": "{{childFill}}", "taskBorderColor": "{{childStroke}}", "taskTextColor": "{{childText}}", "taskTextOutsideColor": "{{childText}}", "taskTextLightColor": "{{childText}}", "taskTextDarkColor": "{{childText}}", "sectionBkgColor": "{{parentFill}}", "sectionBkgColor2": "{{childFill}}", "gridColor": "{{lineColor}}"}}}%%
gantt
    accTitle: FrameMaker content restructuring and DITA XML chain implementation
    accDescr: Diagram showing FrameMaker content restructuring running alongside implementing the DITA XML chain, as two parallel activities.
    dateFormat YYYY-MM-DD
    axisFormat %d
    section Restructuring
    FrameMaker content restructuring         :r1, 2024-01-01, 3d
    Implementation of the DITA XML chain     :r2, 2024-01-01, 3d`} />
...
<Mermaid code={`%%{init: {"flowchart": {"curve": "basis"}, "themeVariables": {"fontFamily": "IBM Plex Sans Variable, sans-serif"}}}%%
graph TD
    accTitle: Application of a conversion table from FrameMaker to DITA XML
    accDescr: Diagram showing FrameMaker content converted to DITA XML by applying a conversion table.

    A[FrameMaker] -->|conversion table| B[DITA XML]
...
The aim of this procedure is to (see also [DITA XML tutorials](../../tutorials/dita-xml-xsl-fo-tutorials/) for setting up the publishing chain):
...
- [From DITA XML to Markdown: lightweight information typing](https://redaction-technique.org/blog/dita-xml-to-markdown-lightweight-information-typing) — applying DITA's information typing in Markdown.
- [Strong information typing without the XML overhead](https://redaction-technique.org/blog/strong-information-typing-without-xml-overhead) — keeping the discipline of structure without XML tooling.
- [Distribute API reference from YAML across docs, and apps](https://redaction-technique.org/blog/experimental-astro-api-docs) — a single structured source feeding multiple outputs.
