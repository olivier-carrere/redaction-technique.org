...
With Jinja, you can define objects (audience, platform, version, etc.) and include or exclude text blocks according to their attributes. For a simpler introductory approach, see [creating different documents from the same sources using Jinja](creer-des-documents-differents-a-partir-des-memes-sources-restructuredtext-jinja-texte-conditionnel/). The same conditional-text concept also applies to [DITA XML's ditaval mechanism](../formats/cas-concret-documentation-de-nufirewall/).
...
- [Create different documents from the same ReST sources (conditional text)](creer-des-documents-differents-a-partir-des-memes-sources-restructuredtext-rest-texte-conditionnel/)
