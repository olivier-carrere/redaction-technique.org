...
| **Generation** | `gen_calendar.py` | Three-class Python script: data loading and remote resolution, calendar coloring, LaTeX emission — each independently readable, testable, and modifiable. Run with `--lang fr`, `--lang en`, or `--lang es`. |
...
The script reads the five YAML files and writes into the language subdirectory to avoid collisions when building multiple editions sequentially:
