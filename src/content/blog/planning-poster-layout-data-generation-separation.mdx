...
> **Free-tier cost.** GitHub Free includes 2,000 Actions minutes per month for private repositories. A LuaLaTeX build for three languages takes several minutes per push; heavy iteration during development eats through that budget fast. The 2,000-minute cap is a real constraint — the `livres-latex` repository hit it in June 2026 during pipeline development. The fix is straightforward: making the repository public removes the minute cap entirely, since GitHub Actions is free for public repositories with no limit. If the content allows it, that is the zero-cost path.
