...
The only file that changes each year is `events.yaml`. Everything else — layout, translations, typography, LaTeX templates — is stable.

### Before touching anything: compare seasons

```bash
make diff-saison SAISON=26-27   # compare 25-26 → 26-27  (PREV= to override)
```

The report lists dates, prices, and events that changed between seasons. Catching a discrepancy here costs nothing; catching it after printing costs a reprint.

### Open a new season block

`events.yaml` is structured as a list of season blocks. Copy the previous block and update three fields:
...
```bash
# Build all three editions — press-ready PDF/X-4 with preflight
make planning planning-en planning-es

# Open for visual check
open planning/fr-planning.pdf planning/en-planning.pdf planning/es-planning.pdf

# Publish
git add planning/events.yaml
git commit -m "Planning: nouvelle saison 27-28"
git push origin main
```

If `gen_calendar.py` reports an `ERREUR — events.yaml`, the field name in the error message identifies exactly where the problem is. Fix it and re-run.
