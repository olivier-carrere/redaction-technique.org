...
The pipeline targets the same technical specifications as the original InDesign files. Each language edition produces a press-ready PDF — double-sided, large-format landscape, with bleed, embedded fonts, and high-resolution images.

| Specification | Value |
|---|---|
| Trim size | A3 landscape — 420 × 297 mm |
| Bleed | 2 mm on each side → MediaBox 424 × 301 mm |
| Crop marks | Not embedded in the PDF; available as a build option |
| Images | 300 ppi, RGB colour space, JPEG compression |
| Fonts | Embedded and subsetted — Exo family (ExtraLight, Regular, Medium, SemiBold, Bold + italics), Verdana (Regular/Bold/Italic), Arial Narrow; fallback if `.ttf` files are absent |
| PDF version | 1.4, tagged (accessibility structure), linearized for progressive web display |
| Metadata | XMP present: title, creator, dates |
| Restrictions | No encryption, forms, JavaScript, or attachments |
| Target file size | ~2–3 MB |

Producing output that matches these specs from YAML sources — fonts subsetted, bleed calculated, metadata populated — is what makes the architecture more than a maintainability improvement: it fully replaces the design tool, not just the data management around it.
