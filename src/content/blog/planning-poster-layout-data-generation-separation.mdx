...
snippet: "How a strict five-layer architecture — factual data, language strings, typographic constants, a Python generator, and a LaTeX form template with no hardcoded values — ensures every fact lives in exactly one place, so a year's update means editing YAML only, never the layout."
image: {
...
The planning enforces a strict five-way separation. Each layer has a single job and no overlap with the others.
...
| **Generation** | `gen_calendar.py` | Reads all YAML files; assembles language-specific LaTeX fragments and a macro file. Run with `--lang fr`, `--lang en`, or `--lang es` for the corresponding edition. |
| **Form** | `fr/planning.tex`, `en/planning.tex`, `es/planning.tex` | Page structure, colours, and fixed editorial text. Calls macros for all data, strings, and layout values. Contains no hardcoded facts, labels, or measurements. |

Because data, language, and typographic values are all in separate YAML files, the same `events.yaml` produces French, English, and Spanish PDFs without alteration. The form layer calls macros such as `\planningyear`, `\inscriptioniban`, `\taxicompagnie`, `\eventfontformat`, `\caldaysetup` — all resolved at build time from the generated macro file. If a value doesn't exist in YAML, it doesn't appear in the PDF. If a value changes in YAML, it changes everywhere in every language edition simultaneously.

---

## How the build flows

The generator (`gen_calendar.py`) runs first, reading five YAML files — three for factual data, one for language strings, one for typographic constants — and optionally fetching live retreat data from the community website. Everything it produces is then consumed by `lang/planning.tex` through standard LaTeX `\input{}` directives.
...
flowchart TD
    ...
    LY[(layout.yaml)] --> G
    G[gen_calendar.py<br/>--lang fr/en/es] --> M[planning_meta.tex<br/>macros for data + layout]
...
The generated `.tex` fragments are committed to the repository alongside the sources, so `lang/planning.tex` compiles without running the Python script at every LaTeX pass. The script only needs to run when data changes.
...
**Typography layer.** `layout.yaml` holds every typographic constant: font sizes and leading for each block, vertical spacing, calendar column widths, tabular parameters. The generator reads these values and exports them as LaTeX macros — `\eventfontformat`, `\caldaysetup`, `\prixtabcolsep` — into `planning_meta.tex`. The form templates contain no hardcoded measurements. Change a font size in `layout.yaml` and all three language editions rebuild with the new value. Like `traduction.yaml`, this file is year-independent: it changes only when typographic choices change, not when the programme does.

**Generation layer.** A single Python script that reads the five YAML files and writes:
- `planning_meta.tex` — one `\def` per data point and one per typographic constant, giving `lang/planning.tex` a named handle for every value.
...
`lang/planning.tex` calls `\inscriptioniban` wherever the IBAN appears. The YAML is the single source; the macro is the interface; the form template never sees the value directly.
...
**Form layer.** Three stable files — `fr/planning.tex`, `en/planning.tex`, `es/planning.tex` — each defines the two-page A4 landscape structure, the colour palette, and the fixed editorial text for its language: descriptions of the practice, directions to the temple, registration instructions. None of that changes year to year. What changes — dates, prices, names, measurements — is not in these files.
...
The three form templates are never touched. The layout — the part that takes effort to get right — stays exactly as it was, for all three language editions simultaneously.
