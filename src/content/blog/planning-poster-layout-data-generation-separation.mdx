...
The failure mode is quiet. A price updated in the sesshin block but missed in the summer camp pricing table. A director listed correctly in the event description but still carrying last year's title in the calendar label. An IBAN corrected in the registration section, intact in the transport section. The document looks finished. The errors are real.
...
The [sesshin planning poster](https://redaction-technique.org/blog/books-as-code-spoken-teachings-latex-ai) for the Yūjō Nyūsanji Zen community solved this by enforcing a rule so strict it sounds simple: the LaTeX source contains **no hardcoded factual data**. Not a single date, price, name, or address.
...
flowchart TD
    ...
    G --> ST[sesshins-temple-fr.tex\ntemple sesshins]
    G --> AS[autres-sesshins-fr.tex\noff-site sesshins]
    G --> CE[camp-ete-*.tex\nsummer camp]
...
- `events.yaml` — all retreat events for the year: dates, dojo, retreat director(s), prices, and an optional `remote_key` field that resolves the date against the live community website.
- `godos.yaml` — a reference list of retreat directors: full name, status (`maître` / `moine` / `nonne`), gender.
...
- Five content fragments covering temple sesshins, off-site sesshins, and summer camp blocks.

The generator is also where data validation lives: mismatched dojo references, missing prices, or ambiguous director names fail here, not silently in the PDF.
...
The [books-as-code pipeline](https://redaction-technique.org/blog/books-as-code-spoken-teachings-latex-ai) for the Shinjinmei volumes uses the same LaTeX infrastructure. The planning poster extends it by adding a data layer thick enough to absorb an entire year's worth of factual change without touching a single line of layout code.
