...
The generated `.tex` fragments are committed to the repository alongside the sources, so `planning.tex` compiles without running the Python script at every LaTeX pass. The script only needs to run when data changes. (The filenames — `sesshins-temple-fr.tex`, `autres-sesshins-fr.tex` — use the original French term; the diagram labels are English glosses of their contents.)
...
The generator reads it and writes this block into `sesshins_temple_block.tex`:
