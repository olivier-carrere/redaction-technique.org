...
> **Building locally (identical to CI).** The CI workflow uses a published TeX Live Docker image. Running the exact same image locally guarantees identical output without a VM:
> ```bash
> docker run --rm -it \
...
>   bash
> ```
> Inside the container: `apt-get update -qq && apt-get install -y --no-install-recommends make python3 python3-yaml && make planning planning-en planning-es`
>
> On macOS ARM64, the image is AMD64 and runs under Rosetta emulation — it works but is slower than native. [OrbStack](https://orbstack.dev/) (`brew install orbstack`) uses Rosetta more efficiently than Docker Desktop; the same `docker run` command works unchanged. The alternative — a UTM Ubuntu ARM64 VM with TeX Live installed manually — would not be bit-for-bit identical to CI and requires more setup.
