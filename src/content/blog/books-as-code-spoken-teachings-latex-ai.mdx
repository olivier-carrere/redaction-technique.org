...
Cover quality and production values don't just affect how the book looks — they affect what it can credibly ask for. An independent AI review of the cover put the appropriate retail price at **25–28 €**: above the 15–22 € range typical for a standard 150-page Buddhist title, but well short of the threshold where name recognition becomes a prerequisite. The reasoning: the cover reads as a reference work rather than a transcription; the QR codes add a dimension no standard print edition provides; a limited posthumous edition carries weight that a mass-market reprint does not. A print run of 200 copies and a community-supported publication model reinforce rather than undercut that positioning. At 28 €, the book's production values and its price are in the same register.
